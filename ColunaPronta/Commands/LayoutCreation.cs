...
namespace LayoutCreation
{
    ...
    public class Commands
    {
        ...
        public void CreateLayout()
        {
            ...
            var center = new Point3d( first.X > second.X ? first.X - ((first.X - second.X) / 2 ): second.X - ( (second.X - first.X) / 2 )
                                    , first.Y > second.Y ? first.Y - ((first.Y - second.Y) / 2 ): second.Y - ( (second.Y - first.Y) / 2 )
                                    ,0
                                    );

            //var ext = new Extents2d();

            Extents2d ext = new Extents2d(first.X, first.Y, second.X, second.Y);

            using (var tr = db.TransactionManager.StartTransaction())
            {
                // Create and select a new layout tab

                var id = LayoutManager.Current.CreateAndMakeLayoutCurrent("NewLayout2");
                ...
                //viewPort.Height = 297;
                //viewPort.Width = 210;
                ////lay.ApplyToViewport(tr, 2, viewPort);
        ...
        // Returns whether the provided DB extents - retrieved 
        // Database.Extmin/max - are "valid" or whether they are the 
        // invalid values (where the min's coordinates are positive and 
